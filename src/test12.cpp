    ...
    { 
        ...
        if(start_flag == true)
        {
            ...
            for(size_t i = 0; i < 2; i++)
            {
                /* --------------------------------- PCD文件读取 -------------------------------- */
                if(i == 0){
                    base_x = rtk_data.easting;
                    base_y = rtk_data.northing;
                    cout << setiosflags(ios::fixed) << setprecision(7) << "基准点：" 
                         << "x:" << base_x 
                         << " y: " << base_y << endl;
                ...
                float point_x = base_x + 3;     // 或者写目标点
                float point_y = base_y + 4;
                cout << setiosflags(ios::fixed) << setprecision(7)  << "目标点  " 
                     << "x:" << point_x 
                     << " y: " << point_y << endl;
                     
                cout << "是否开始跟踪？(1-开始   0-退出): ";
                int tmp = -1;
                cin >> tmp;
                if(tmp==0){
                    exit(0);
                ...
                // 先预读一次，更新最新的自身的位置，并计算距离，如果下一个点足够近，就不用动
                ros::spinOnce();
